...
<form action="../controllers/update-customer.php?id=<?php echo $user->id; ?>" method="post">
    ...
    <select name="plan"
        ...
        <option value="<?php echo (int)$my_plan->id; ?>" selected>Piano <?php echo (int)$my_plan->id; ?></option>
        <?php if (!$is_plan_valid) {
            while ($plan = $all_plans->fetch_assoc()) {
                if ((int)$plan['id'] !== (int)$my_plan->id) { ?>
                    <option value="<?php echo (int)$plan['id']; ?>">Piano <?php echo (int)$plan['id']; ?></option>
                <?php }
            }
        } ?>
    </select>
    ...
    if ($is_plan_valid) {
        ?>
        <input type="hidden" id="sport_option" name="sport_option" value="<?php echo $my_plan->opzione_sport; ?>"/>
        <?php
    }
    ?>
                ...
                <li>
                    ...
                    <ul>
                        ...
                        if ($content['durata'] !== null) { ?>
                            <li>Durata:<strong> <?php echo $content['durata']; ?></strong></li>
                        <?php }
